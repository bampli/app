...
# **search_facility**
> list[Facility] search_facility(size=size, page=page, sort=sort, name=name, companyid=companyid)
...
try:
    # Load the list of Facilities
    api_response = api_instance.search_facility(size=size, page=page, sort=sort, name=name, companyid=companyid)
    pprint(api_response)
except ApiException as e:
